...
export const WaitingLobby = (): JSX.Element | null => {
    ...
    const allReady =
        ...
        playersList.every((player) => Boolean(player.isReady));

    //UseEffect para debuggear el estado del lobby
    useEffect(() => {
        console.log("WaitingLobby debug:", {
            userName,
            isCurrentUserMaster,
            playersList,
            allReady,
        });
    }, [userName, isCurrentUserMaster, playersList, allReady]);
            ...
            <div className="p-6">
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-6">
                    {playersList.map((player) => (
                        <CustomTooltip
                            key={player.id}
                            text="Jugadores en la sala. Haz clic para indicar que estás listo."
                        >
                            <div
                ...
                <div className="flex justify-between items-center gap-2">
                    ...
                    {isCurrentUserMaster ? (
                        ...
                        <Loader2 className="text-sm text-gray-400 mr-2 animate-spin"></Loader2>
                    )}
                </div>
            </div>
        </div>
        ...
        <div className="w-full">
            ...
            <div className="p-6 space-y-4">
                ...
                <div className="flex gap-2 pt-2">
                    <button onClick={createRoom} className="flex-1 btn-primary">
                        Create Room
                    </button>
                    <button onClick={joinRoom} className="flex-1 btn-success">
                        Join Room
                    </button>
                </div>
            </div>
